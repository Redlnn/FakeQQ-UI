...
function play() {
  if (audioCtx === undefined || audioBuffer === undefined || progressItemsRefs.value === undefined)
    return
  ...
  if (playEnded.value) {
    ...
    progressItemsRefs.value.style.setProperty('--process-item-color', 'var(--qq-text-secondary-01)')
    ...
    const { pause: pauseBar, resume: resumeBar } = useIntervalFn(
      () => {
        if (playEnded.value) {
          ...
          progressItemsRefs.value?.style.setProperty(
            '--process-item-color',
            'var(--qq-text_primary)'
          )
        }
